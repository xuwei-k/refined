...
object collection extends CollectionInference {
  ...
  object Count {
    inline given countValidate[A, PA, RA, PC, RC, T](using
        va: Validate.Aux[A, PA, RA],
        vc: Validate.Aux[Int, PC, RC],
        ev: T => Iterable[A]
    ): Validate.Aux[T, Count[PA, PC], Count[List[va.Res], vc.Res]] =
      new CountValidateImpl[A, PA, RA, PC, RC, T]

    final class CountValidateImpl[A, PA, RA, PC, RC, T](using
        va: Validate.Aux[A, PA, RA],
        vc: Validate.Aux[Int, PC, RC],
        ev: T => Iterable[A]
      ...
      private inline def count(t: T): Int =
        ev(t).count(x => va.isValid(x))
    }
  }
